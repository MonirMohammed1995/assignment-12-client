...
const Navbar = () => {
  ...
  const navLinks = (
    <>
      ...
      {user && role === 'user' && (
        <NavLink to="/dashboard/user" className={({ isActive }) => isActive ? "text-primary font-semibold" : "hover:text-primary transition"}>User Dashboard</NavLink>
      )}
      {user && role === 'admin' && (
        <NavLink to="/dashboard/admin" className={({ isActive }) => isActive ? "text-primary font-semibold" : "hover:text-primary transition"}>Admin Dashboard</NavLink>
      )}
    </>
  );
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-base-100 shadow-md' : 'bg-transparent'}`}>
      <nav className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="md:hidden dropdown dropdown-end">
          ...
          <ul
            tabIndex={0}
            className="menu dropdown-content mt-3 z-[99] p-3 shadow bg-base-100 rounded-box w-52 space-y-1"
          >
            {navLinks}
            <li>
