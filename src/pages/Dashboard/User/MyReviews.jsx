...
const MyReviews = () => {
  ...
  return (
    <div className="p-6 bg-gray-50 shadow-sm rounded-xl max-w-7xl mx-auto mb-16">
      <h2 className="text-3xl font-bold mb-6 text-indigo-700 text-center">My Reviews</h2>
      ...
      {reviews.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-lg shadow-md bg-white">
          <table className="min-w-full border-collapse">
            <thead className="bg-indigo-100 text-indigo-700 text-left text-sm uppercase">
              <tr>
                <th className="p-3 border border-indigo-200">Scholarship</th>
                <th className="p-3 border border-indigo-200">University</th>
                <th className="p-3 border border-indigo-200">Comment</th>
            ...
            <tbody className="text-gray-700">
              {reviews.map((review) => (
                <tr key={review._id} className="hover:bg-indigo-50 transition-colors">
                  ...
                  <td className="p-3 border border-indigo-200 text-center flex flex-col sm:flex-row justify-center gap-2">
                    <button
                      onClick={() => setEditingReview(review)}
                      className="px-4 py-1 bg-yellow-500 hover:bg-yellow-600 text-white rounded-md transition"
                    >
                      Edit
                    </button>
                    <button
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-gray-300 p-2 rounded-lg resize-y focus:outline-none focus:ring-2 focus:ring-indigo-400"
                  required
                  rows={4}
                />
              </div>
